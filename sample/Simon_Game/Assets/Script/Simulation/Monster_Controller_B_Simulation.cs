...
public class Monster_Controller_B_Simulation : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if (GameTimeManager_Simulation.GroupB.Count == NormalGameSceneManager_Simulation.Total_Object) 
		{
			for(int i= 0 ; i < GameTimeManager_Simulation.GroupB.Count ; i++)
			{
				if(((SIMONObject)GameTimeManager_Simulation.GroupB.ValueOfIndex (i)).ObjectID.Equals(gObj_Monster.name))
				{
					sObject = ((SIMONObject)GameTimeManager_Simulation.GroupB.ValueOfIndex (i));
					reStart();
					return;
				}
			}
		...
		GameTimeManager_Simulation.GroupB.Add (sObject.ObjectID, sObject);
	...
	//	Range.Inherit = true;
		...
		//
		SIMONFunction ObjectFitness_Name = ObjectFitness;
		...
		SIMON.GlobalSIMON.InsertSIMONMethod("ObjectFitness" , ObjectFitness_Name);
		//
