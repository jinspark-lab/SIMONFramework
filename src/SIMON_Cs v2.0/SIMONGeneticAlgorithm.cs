...
{
    /// <summary>
    /// SIMONAlgorithm 인터페이스를 구현하는 유전 알고리즘 클래스입니다.
    /// </summary>
    public class SIMONGeneticAlgorithm : SIMONAlgorithm
    {
        ...
        /// <summary>
        /// 유전자와 해당 유전자가 갖는 FitnessValue를 갖는 구조체입니다.
        /// </summary>
        ...
        /// <summary>
        /// 유전 알고리즘의 Option을 설정할 수 있는 클래스입니다.
        /// </summary>
        ...
        /// 돌연변이 발생 확률을 설정합니다.
        /// </summary>
        /// <param name="mutateChance">돌연변이 발생확률입니다.</param>
        ...
        /// 돌연변이 변이 정도를 설정합니다.
        /// </summary>
        /// <param name="mutateProportion">돌연변이 변화율입니다.</param>
        ...
        public object Implementation(SIMONCollection GroupCollection, SIMONCollection SimonFunctions)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions)
//        public object Implementation(SIMONCollection GroupCollection, Dictionary<string, SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate> SimonFunctions)
        {
            try
            {
        ...
        /// <returns>구현에 대한 결과값입니다.</returns>
        public object Implementation(SIMONCollection GroupCollection, SIMONCollection SimonFunctions, double learningRate)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions, double learningRate)
        {
            List<List<SIMONGene>> selectedDNAPool = Selection(GroupCollection, SimonFunctions);
            List<List<SIMONGene>> crossedDNAPool = CrossOver(selectedDNAPool);
            List<List<SIMONGene>> mutatedDNAPool = Mutation(crossedDNAPool);
        ...
        /// <returns>구현에 대한 결과값입니다.</returns>
        public object Implementation(SIMONCollection GroupCollection, SIMONCollection ActionMap, SIMONCollection SimonFunctions)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions)
//        public object Implementation(SIMONCollection GroupCollection, SIMONCollection ActionMap, Dictionary<string, SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate> SimonFunctions)
        {
        ...
        /// <returns>구현에 대한 결과값입니다.</returns>
        public object Implementation(SIMONCollection GroupCollection, SIMONCollection ActionMap, SIMONCollection SimonFunctions, double learningRate)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions, double learningRate)
//        public object Implementation(SIMONCollection GroupCollection, SIMONCollection ActionMap, Dictionary<string, SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate> SimonFunctions, double learningRate)
        {
            List<List<List<SIMONGene>>> selectedDNAPool = Selection(GroupCollection, ActionMap, SimonFunctions);
            List<List<List<SIMONGene>>> crossedDNAPool = CrossOver(selectedDNAPool);
            List<List<List<SIMONGene>>> mutatedDNAPool = Mutation(crossedDNAPool);
        ...
        public List<List<SIMONGene>> Selection(SIMONCollection PropertyCollection, SIMONCollection SimonFunctions)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions)
        {
            ...
            try
            {
                ...
                {
                    SIMONGeneticObject elementObject = (SIMONGeneticObject)PropertyCollection.ValueOfIndex(i);
                    SIMONGeneticObject[] otherObjects = new SIMONGeneticObject[dnaObjectCount - 1];
                    int otherObjectsCnt = 0;
                    propertyGenes[i] = new GeneValue();

                    //otherObject 리스트 추가시키면됨.
                    for (int j = 0; j < dnaObjectCount; j++)
                        if (elementObject != (SIMONGeneticObject)PropertyCollection.ValueOfIndex(j))
                            otherObjects[otherObjectsCnt++] = (SIMONGeneticObject)PropertyCollection.ValueOfIndex(j);

                    propertyGenes[i].dna = elementObject.PropertyDNA;
                    double fitnessValue = (double)((SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate)((DictionaryEntry)SimonFunctions[elementObject.ObjectFitnessFunctionName]).Value).Invoke(elementObject, otherObjects);
        ...
        public List<List<List<SIMONGene>>> Selection(SIMONCollection ObjectCollection, SIMONCollection ActionMap, SIMONCollection SimonFunctions)//Dictionary<string, SIMONFunctionInterface<SIMONGenetics>.SIMONDelegate> SimonFunctions)
//        public List<List<List<SIMONGene>>> Selection(SIMONCollection ObjectCollection, SIMONCollection ActionMap, Dictionary<string, SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate> SimonFunctions)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ActionCount; i++)
                {
                    List<SIMONGeneticObject> elementList = new List<SIMONGeneticObject>();
                    //List<dynamic> recastList = (SIMONObjectFactory.RecastObjects<SIMONGeneticProperty, SIMONGeneticAction>(ActionMap.ValueOfIndex(i)));
                    //elementList = recastList.Cast<SIMONGeneticObject>().ToList();

                    elementList = ((List<object>)ActionMap.ValueOfIndex(i)).Cast<SIMONGeneticObject>().ToList();
                    ...
                    for (int j = 0; j < actionObjectCount; j++)
                    {
                        ...
                        SIMONGeneticObject[] otherObjectsList;
                        if (ObjectCount > 1)
                            otherObjectsList = new SIMONGeneticObject[ObjectCollection.Count - 1];
                        else
                            otherObjectsList = null;
                        int otherObjectCnt = 0;

                        for (int k = 0; k < ObjectCount; k++)
                            if ((otherObjectsList != null) && (!(ObjectCollection.ValueOfIndex(k).ObjectID.Equals(elementList[j].ObjectID))))
                                otherObjectsList[otherObjectCnt++] = (SIMONGeneticObject)ObjectCollection.ValueOfIndex(k);
                        ...
                        for (int k = 0; k < elementList[j].Actions.Count; k++)
                        {
                            if (elementList[j].Actions[k].ActionName.Equals(ActionMap.KeyOfIndex(i)))
                            {
                                ...
                                double fitnessValue = (double)((SIMONFunctionInterface<SIMONGeneticProperty, SIMONGeneticAction>.SIMONDelegate)((DictionaryEntry)SimonFunctions[elementList[j].Actions[k].FitnessFunctionName]).Value).Invoke(elementList[j], otherObjectsList);
                ...
                for (int i = 0; i < ActionCount; i++)
                {
                    List<SIMONGeneticObject> actionObjects = new List<SIMONGeneticObject>();
                    //List<dynamic> recastObjects = SIMONObjectFactory.RecastObjects<SIMONGeneticProperty, SIMONGeneticAction>(ActionMap.ValueOfIndex(i));
                    //actionObjects = recastObjects.Cast<SIMONGeneticObject>().ToList();

                    actionObjects = ((List<object>)ActionMap.ValueOfIndex(i)).Cast<SIMONGeneticObject>().ToList();


                    //((List<object>)ActionMap.ValueOfIndex(i)).Cast<SIMONGeneticObject>().ToList()[j].Actions[k].ActionDNA = nextDNA[i][geneIdx];

                    int numberOfActionObjects = actionObjects.Count;
        ...
        /// </summary>
        /// <param name="geneSet">RouletteWheel을 적용할 유전자셋입니다.</param>
        /// <param name="law">Roulette에 사용될 유전 법칙입니다.</param>
        /// <returns>Randomize하게 선택된 유전자셋.</returns>
        ...
        /// <returns>교차된 2차원 유전자 풀입니다.</returns>
        ...
        /// <returns>교차된 3차원 유전자 풀입니다.</returns>
        ...
        /// <returns>변이된 2차원 유전자 풀입니다.</returns>
        ...
        /// <returns>변이된 3차원 유전자 풀입니다.</returns>
        ...
        private void Evolution(SIMONCollection PropertyCollection, List<List<SIMONGene>> nextDNA)
        {
            ...
            for (int i = 0; i < objectCount; i++)
            {
                ...
                {
                    for (int k = 0; k < ((SIMONGeneticObject)PropertyCollection.ValueOfIndex(i)).PropertyDNA.Count; k++)
                    {
                        if (((SIMONGeneticObject)PropertyCollection.ValueOfIndex(i)).PropertyDNA[k].ElementName == nextDNA[selectDNAIdx][j].ElementName)
                        {
                            ((SIMONGeneticObject)PropertyCollection.ValueOfIndex(i)).PropertyDNA[k] = nextDNA[selectDNAIdx][j];
                        }
                    }
                }
            }
        ...
        private void Evolution(SIMONCollection PropertyCollection, List<List<SIMONGene>> nextDNA, double learningRate)
        {
            ...
            {
                int selectDNAIdx = rand.Next(0, nextDNA.Count - 1);
                ((SIMONGeneticObject)PropertyCollection.ValueOfIndex(i)).PropertyDNA = nextDNA[selectDNAIdx];
            }
        }
        ...
        private void Evolution(SIMONCollection ObjectCollection, SIMONCollection ActionMap, List<List<List<SIMONGene>>> nextDNA)
        {
            ...
            for (int i = 0; i < ActionMap.Count; i++)
            {
                ...
                List<SIMONGeneticObject> actionObjectList = new List<SIMONGeneticObject>();
                List<dynamic> recastObjects = SIMONObjectFactory.RecastObjects<SIMONGeneticProperty, SIMONGeneticAction>(ActionMap.ValueOfIndex(i));
                actionObjectList = recastObjects.Cast<SIMONGeneticObject>().ToList();
        ...
        private void Evolution(SIMONCollection ObjectCollection, SIMONCollection ActionMap, List<List<List<SIMONGene>>> nextDNA, double learningRate)
        {
            ...
            for (int i = 0; i < ActionMap.Count; i++)
            {
                ...
                List<SIMONGeneticObject> actionObjectList = (List<SIMONGeneticObject>)ActionMap.ValueOfIndex(i);
                ...
                for (int j = 0; j < actionObjectList.Count; j++)
                {
                    for (int k = 0; k < actionObjectList[j].Actions.Count; k++)
                    {
                        ...
                        {
                            int geneIdx = rand.Next(0, nextDNA[i].Count);
                            ((List<SIMONGeneticObject>)ActionMap.ValueOfIndex(i))[j].Actions[k].ActionDNA = nextDNA[i][geneIdx];
                        }
                    }
                }
            }
        ...
        private void UpdateObjectsPropertyDNA(SIMONCollection ObjectCollection)
        {
            ...
            {
                SIMONGeneticObject elementObject = (SIMONGeneticObject)ObjectCollection.ValueOfIndex(i);
