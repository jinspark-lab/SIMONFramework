...
{
    /// <summary>
    /// Key값과 Value값을 순서쌍으로 갖는 SIMONObjectCollection Dictionary ADT를 제공합니다.
    /// </summary>
    public class SIMONObjectCollection : IDictionary
    {
        //IDictionary 인터페이스를 상속받아서 Key, Value 순서쌍을 이루는 자료구조의 래퍼 컬렉션을 구현한다. SIMONObject 들을 관리하기 위한 특수 메서드 들도 구현해야 한다.
        public DictionaryEntry[] SIMONDictionary { get; private set; }
        public int CollectionSize { get; private set; }
        private object collectionSyncLocker;                                            //Collection에 대한 쓰레드 동기화시 사용할 Locker Object를 정의한다.

        #region IDictionary Interface 속성
        public bool IsFixedSize { get; private set; }                                   //컬렉션의 크기가 정해져있는지를 검사한다.
        public bool IsReadOnly { get; private set; }                                    //컬렉션이 읽기 전용인지를 검사한다.
        public ICollection Keys { get; private set; }                                   //컬렉션 내의 모든 키를 나열한다.
        public ICollection Values { get; private set; }                                 //컬렉션 내의 모든 값들을 나열한다.
        public int Count { get; private set; }                                          //컬렉션 내에서 사용하는 요소들의 수를 저장한다.
        #endregion
        ...
        public object ValueOfIndex(int index){
            ...
            {
                throw new IndexOutOfRangeException("Input index number is out of dictionary's range");
            }
        ...
        public void Add(object key, object value)
        {
            ...
            if (key == null)
                throw new ArgumentNullException("key", "Invalid Key");
            if (value == null)
                throw new ArgumentNullException("value", "Invalid Value");
            if (Count.Equals(SIMONDictionary.Length))
            {
                //현재 들어있는 엘리먼트의 갯수가 길이와 같을경우(가득 차있는 경우), Array Doubling 작업 수행하고, value를 푸시해줘야함. 
                if (!ArrayDoubling())
                    throw new IndexOutOfRangeException("SIMONCollection has been corrupted. Index out of range");
        ...
        public void Remove(object key)
        {
            ...
            if (index != SIMONConstants.INVALID_COLLECTION_INDEX)
            {
                ...
                throw new ArgumentNullException("key", "There is no value in the dictionary matched with argument 'key'");              //키가 Dictionary에 없을 경우 예외 반환.
        }
        ...
        public void CopyTo(Array array, int index)
        {
            ...
            throw new NotImplementedException("SIMON Collection can not be copied to another space directly.");
        }
        ...
        private class SIMONObjectCollectionEnumerator : IDictionaryEnumerator
        {
            ...
            public SIMONObjectCollectionEnumerator(SIMONObjectCollection simonCollection)
            {
