...
{
    /// <summary>
    /// Genetic Algorithm을 이용해서 SIMONFramework의 AI를 적용시킬 객체를 정의합니다. Manager를 통해 관리됩니다.
    /// </summary>
    [Serializable]
    [XmlRoot("SIMON")]
    public class SIMONGeneticObject : SIMONObject<SIMONGeneticProperty, SIMONGeneticAction> 
    {
        ...
        public List<SIMONGeneticProperty> Properties { get; set; }

        /// <summary>
        /// SIMONObject가 갖는 Action 집합을 정의합니다.
        /// </summary>
        [XmlElement("Actions")]
        public List<SIMONGeneticAction> Actions { get; set; }
        ...
        public SIMONGeneticObject()
        {
            Properties = new List<SIMONGeneticProperty>();
            Actions = new List<SIMONGeneticAction>();
        ...
        public void LoadObjectDefinition(string projectName, string fileName)
        {
            ...
            SIMONGeneticObject xmlObject = (SIMONGeneticObject)SIMONUtility.GetInstance().DeserializeObject(definitionPath, SIMONObjectType.SIMONGeneticObject);
            SIMONGeneticObject copiedObject = SIMONObjectFactory.CopyObject((SIMONGeneticObject)xmlObject);

            this.ObjectID = copiedObject.ObjectID;
            this.Properties = copiedObject.Properties;
            this.Actions = copiedObject.Actions;
            this.PropertyDNA = copiedObject.PropertyDNA;
        }
        ...
        {
            string definitionPath = @"\" + projectName + @"\" + fileName;
            SIMONGeneticObject thisObject = SIMONObjectFactory.CopyObject(this);
        ...
        public SIMONGeneticAction GetActionObject(string actionName)
        {
            foreach (SIMONGeneticAction actionElement in Actions)
            {
        ...
        {
            foreach (SIMONGeneticProperty sProp in Properties)
            {
    ...
    public class SIMONGeneticProperty : SIMONProperty
    {
        ...
        public SIMONGeneticProperty()
        {
            //Deserialize시 Parameter를 보증하기 위한 private parameterless 생성자. 실제 코드상에서 클래스를 이용할 때는 쓰이지 않는다.
            PropertyName = "NULL";
            PropertyValue = 0.0;
        }
        public SIMONGeneticProperty(string propertyName, double propertyValue, bool inherit)
        {
    ...
    public class SIMONGeneticAction : SIMONAction
    {
        /// <summary>
        ...
        public SIMONGeneticAction()
        {
            ActionDNA = new List<SIMONGene>();
        }
        public SIMONGeneticAction(string actionName, string actionFunctionName, string executionFunctionName, string fitnessFunctionName, List<SIMONGene> actionDNA)
        {
