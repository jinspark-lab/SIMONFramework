...
public class UsualAI_Controller : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if (GameTimeManager_Compare.GroupB.Count == 7) 
		{
			for(int i= 0 ; i < GameTimeManager_Compare.GroupB.Count ; i++)
			{
				if(((SIMONObject)GameTimeManager_Compare.GroupB.ValueOfIndex (i)).ObjectID.Equals(sObject.ObjectID))
				{
					sObject = ((SIMONObject)GameTimeManager_Compare.GroupB.ValueOfIndex (i));
					reStart();
