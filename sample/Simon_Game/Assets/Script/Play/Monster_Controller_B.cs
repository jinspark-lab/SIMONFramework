...
public class Monster_Controller_B : MonoBehaviour 
{
	...
	void OnGUI() {
		...
		if (!NormalGameSceneManager.BoostAtResultPage)
		{
	...
	void Start () 
	{
		...
		if (GameTimeManager.GroupB.Count == 7) 
		{
			for(int i= 0 ; i < 7 ; i++)
			{
				if(((SIMONObject)GameTimeManager.GroupB.ValueOfIndex (i)).ObjectID.Equals(sObject.ObjectID))
				{
					sObject = ((SIMONObject)GameTimeManager.GroupB.ValueOfIndex (i));
					reStart();
	...
	public System.Object ActionFunction_Attack(SIMONObject a, SIMONObject[] b)
	{
		...
		double tempValue = 0.0d;
		double hp =a.GetPropertyElement("CurHp");
		double damage = a.GetPropertyElement ("Strength")*5;
		double attack_Speed = a.GetPropertyElement ("Monster_Attack_Speed");
		double attackedCount = a.GetPropertyElement ("AttackedCount");
		double defensive = a.GetPropertyElement("Defensive");
		double total_hp = a.GetPropertyElement ("HP");
		...
		if(player_hp > 0 && a.GetPropertyElement("Type") == 1)
		{
			...
			{
				MaxValue =((scoreConversion(-1.0d * player_total_hp,player_hp)*geneA)+
				           (scoreConversion(-7.0d,a.GetPropertyElement("AttackedCount"))*geneC)+
				           (scoreConversion(-19.0d,dis)*geneD)
				           );
				a.SetPropertyElement("Attack_Target",0.0d);
				a.SetPropertyElement("Monster_Enemy_Defensive",Target_Temp.GetComponent<Player_Controller> ().Player_Defensive);
			}
		}
		for (int i = 0; i < b.Length; i++) 
		{
	...
	public void Monster_defense ()
	{
		...
		{
		case 1:
			animator.Play ("Ani_Defense_Up_B");
			
			break;
		case 2:
			animator.Play("Ani_Defense_Right_B");
			break;
		case 3:
			animator.Play ("Ani_Defense_Down_B");
			break;
		case 4:
			animator.Play ("Ani_Defense_Left_B");
			break;
		}
	}
	...
	public void Monster_move()
	{
		...
		{
			if(sObject.GetPropertyElement("Target") ==0)
				gObj_Target_Move = GameObject.Find ("Player");
			else
				gObj_Target_Move = GameObject.Find ("Monster_A_"+sObject.GetPropertyElement("Target").ToString());
			Monster_Move_Ani ();
	...
	public void Monster_attack()
	{
		   ...
		   (sObject.GetPropertyElement("isCheck") == 1  || sObject.GetPropertyElement("State") == 1))
		{
			...
			bool check  = false;
			if(sObject.GetPropertyElement("Target") ==0)
				gObj_Target_Attack = GameObject.Find ("Player");
			else
				gObj_Target_Attack = GameObject.Find ("Monster_A_"+sObject.GetPropertyElement("Target").ToString());
			...
			{
			case 1:
				animator.Play ("Ani_Attack_Up_B");
				
				break;
			case 2:
				animator.Play("Ani_Attack_Right_B");
				break;
			case 3:
				animator.Play ("Ani_Attack_Down_B");
				break;
			case 4:
				animator.Play ("Ani_Attack_Left_B");
				break;
			}
	...
	public void Monster_Avoid_Ani(float x,float y)
	{
		...
		{
		case 1:
			animator.Play ("Ani_Move_Up_B");
			break;
		case 2:
			animator.Play ("Ani_Move_Right_B");
			break;
		case 3:
			animator.Play ("Ani_Move_Down_B");
			break;
		case 4:
			animator.Play ("Ani_Move_Left_B");
			break;
		}
	}
