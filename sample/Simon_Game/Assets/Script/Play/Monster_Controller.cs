...
public class Monster_Controller : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		if (GameTimeManager.GroupA.Count == 7) 
		{
			for(int i= 0 ; i < 7 ; i++)
			{
				if(((SIMONObject)GameTimeManager.GroupA.ValueOfIndex (i)).ObjectID.Equals(sObject.ObjectID))
				{
					sObject = ((SIMONObject)GameTimeManager.GroupA.ValueOfIndex (i));
					reStart();
	...
	public System.Object PropertyUpdate(SIMONObject a, SIMONObject[] b)
	{
		...
		{
			if (myObject.GetComponent<Monster_Controller>().animator.GetCurrentAnimatorStateInfo (0).IsTag ("Stay")) 
			{
		...
		{
			if (myObject.GetComponent<Monster_Controller_B>().animator.GetCurrentAnimatorStateInfo (0).IsTag ("Stay")) 
			{
				a.SetPropertyElement("isCheck",1);
			}
		}
